...
class TenistasServiceImplTest {
    ...
    fun `getAll debe retornar lista de tenistas localmente`() = runTest {
        coEvery { localRepository.getAll().await() } returns Ok(listOf(testTenista))

        val result = service.getAll(false).await()
        ...
        coVerify(atLeast = 1) { localRepository.getAll().await() }
    }
    ...
    fun `getAll lista de tenistas remota`() = runTest {
        coEvery { remoteRepository.getAll().await() } returns Ok(listOf(testTenista))
        coEvery { localRepository.saveAll(any()).await() } returns Ok(1)
        coEvery { localRepository.removeAll().await() } returns Ok(Unit)

        val result = service.getAll(true).await()
        ...
        coVerify(atLeast = 1) { remoteRepository.getAll().await() }
        coVerify(atLeast = 1) { localRepository.saveAll(any()).await() }
        coVerify(atLeast = 1) { localRepository.removeAll().await() }
    }
    ...
    fun `getById debe retornar tenista en cache`() = runTest {
        ...
        val result = service.getById(id).await()
    ...
    fun `getById debe retornar tenista local si no esta en cache`() = runTest {
        ...
        coEvery { localRepository.getById(id).await() } returns Ok(testTenista)
        ...
        verify(atLeast = 1) { cache.get(id) }
        coVerify(atLeast = 1) { localRepository.getById(id).await() }
    ...
    fun `getById debe retornar tenista remoto si no esta en cache ni local`() = runTest {
        val id = testTenista.id
        every { cache.get(id) } returns null
        coEvery { localRepository.getById(id).await() } returns Err(TenistaError.NotFound(id))
        coEvery { remoteRepository.getById(id).await() } returns Ok(testTenista)
        every { cache.put(id, testTenista) } returns Unit
        coEvery { localRepository.save(testTenista).await() } returns Ok(testTenista)
        ...
        verify(atLeast = 1) { cache.get(id) }
        coVerify(atLeast = 1) { localRepository.getById(id).await() }
        coVerify(atLeast = 1) { remoteRepository.getById(id).await() }
        verify(atLeast = 1) { cache.put(id, testTenista) }
        coVerify(atLeast = 1) { localRepository.save(testTenista).await() }
    }
    ...
    fun `getById debe retornar error si no esta en cache ni local ni remoto`() = runTest {
        ...
        every { cache.get(id) } returns null
        coEvery { localRepository.getById(id).await() } returns Err(TenistaError.NotFound(id))
        coEvery { remoteRepository.getById(id).await() } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `save debe retornar tenista guardado`() = runTest {
        ...
        coEvery { localRepository.save(newTenista).await() } returns Ok(newTenista)
        coEvery { remoteRepository.save(newTenista).await() } returns Ok(newTenista)
        every { cache.put(newTenista.id, newTenista) } returns Unit
        coEvery { notificationsService.send(any()) } returns Unit

        val result = service.save(newTenista).await()
        ...
        coVerify(atLeast = 1) { localRepository.save(newTenista).await() }
        coVerify(atLeast = 1) { remoteRepository.save(newTenista).await() }
    ...
    fun `save debe retornar error si no se puede guardar por validacion`() = runTest {
        ...
        coVerify(atLeast = 0) { localRepository.save(newTenista).await() }
        coVerify(atLeast = 0) { remoteRepository.save(newTenista).await() }
    }
    ...
    fun `save debe retornar error si no se puede guardar remotamente`() = runTest {
        ...
        coEvery { remoteRepository.save(newTenista).await() } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `update debe retornar tenista actualizado`() = runTest {
        ...
        coEvery { localRepository.update(updatedTenista.id, updatedTenista).await() } returns Ok(updatedTenista)
        coEvery { remoteRepository.update(updatedTenista.id, updatedTenista).await() } returns Ok(updatedTenista)
        every { cache.put(updatedTenista.id, updatedTenista) } returns Unit
        coEvery { notificationsService.send(any()) } returns Unit

        val result = service.update(updatedTenista.id, updatedTenista).await()
        ...
        coVerify(atLeast = 1) { localRepository.update(updatedTenista.id, updatedTenista).await() }
        coVerify(atLeast = 1) { remoteRepository.update(updatedTenista.id, updatedTenista).await() }
    ...
    fun `update debe retornar error si no se puede actualizar por validacion`() = runTest {
        ...
        coVerify(atLeast = 0) { localRepository.update(updatedTenista.id, updatedTenista).await() }
        coVerify(atLeast = 0) { remoteRepository.update(updatedTenista.id, updatedTenista).await() }
        verify(atLeast = 1) { cache.get(updatedTenista.id) }
    }
    ...
    fun `update debe retornar error si no se puede actualizar remotamente`() = runTest {
        ...
        } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `update debe retornar error porque el tenista no existe remotamente`() = runTest {
        ...
        coEvery {
            localRepository.getById(updatedTenista.id).await()
        } returns Err(TenistaError.NotFound(updatedTenista.id))
        coEvery { remoteRepository.getById(updatedTenista.id).await() } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `delete debe retornar tenista borrado`() = runTest {
        ...
        // Suponemos que está en la cache
        every { cache.get(id) } returns testTenista
        coEvery { localRepository.delete(id).await() } returns Ok(id)
        coEvery { remoteRepository.delete(id).await() } returns Ok(id)
        every { cache.remove(id) } returns Unit
        coEvery { notificationsService.send(any()) } returns Unit

        val result = service.delete(id).await()
        ...
        coVerify(atLeast = 1) { localRepository.delete(id).await() }
        coVerify(atLeast = 1) { remoteRepository.delete(id).await() }
    ...
    fun `delete debe retornar error si no se puede borrar remotamente`() = runTest {
        ...
        every { cache.get(id) } returns testTenista
        coEvery { localRepository.delete(id).await() } returns Ok(id)
        coEvery { remoteRepository.delete(id).await() } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `delete debe retornar error si no se puede borrar localmente`() = runTest {
        ...
        every { cache.get(id) } returns testTenista
        coEvery { remoteRepository.delete(id).await() } returns Ok(id)
        coEvery { localRepository.delete(id).await() } returns Err(TenistaError.NotFound(id))
    ...
    fun `import debe devolver Ok para fichero CSV`() = runTest {
        ...
        coEvery { csvStorage.import(file).await() } returns Ok(listOf(testTenista))
        coEvery { localRepository.removeAll().await() } returns Ok(Unit)
        coEvery { remoteRepository.save(testTenista).await() } returns Ok(testTenista)
        coEvery { localRepository.save(testTenista).await() } returns Ok(testTenista)

        val result = service.import(file).await()
        ...
        coVerify(atLeast = 1) { csvStorage.import(file).await() }
        coVerify(atLeast = 1) { localRepository.removeAll().await() }
        coVerify(atLeast = 1) { remoteRepository.save(testTenista).await() }
    }
    ...
    fun `import debe devolver Ok para fichero JSON`() = runTest {
        ...
        coEvery { jsonStorage.import(file).await() } returns Ok(listOf(testTenista))
        coEvery { localRepository.removeAll().await() } returns Ok(Unit)
        coEvery { remoteRepository.save(testTenista).await() } returns Ok(testTenista)
        coEvery { localRepository.save(testTenista).await() } returns Ok(testTenista)
        ...
        coVerify(atLeast = 1) { jsonStorage.import(file).await() }
        coVerify(atLeast = 1) { localRepository.removeAll().await() }
        coVerify(atLeast = 1) { remoteRepository.save(testTenista).await() }
    }
    ...
    fun `import debe devolver error si no se puede importar`() = runTest {
        ...
        coEvery { csvStorage.import(file).await() } returns Err(TenistaError.StorageError("Error"))
    ...
    fun `export debe devolver Ok para fichero CSV de manera local`() = runTest {
        ...
        coEvery { localRepository.getAll().await() } returns Ok(listOf(testTenista))
        coEvery { csvStorage.export(file, listOf(testTenista)).await() } returns Ok(1)

        val result = service.export(file, false).await()
        ...
        coVerify(atLeast = 1) { localRepository.getAll().await() }
        coVerify(atLeast = 1) { csvStorage.export(file, listOf(testTenista)).await() }
    }
    ...
    fun `export debe devolver Ok para fichero CSV de manera remota`() = runTest {
        ...
        val result = service.export(file, true).await()
    ...
    fun `export debe devolver Ok para fichero Json de manera local`() = runTest {
        ...
        coEvery { localRepository.getAll().await() } returns Ok(listOf(testTenista))
        coEvery { jsonStorage.export(file, listOf(testTenista)).await() } returns Ok(1)
        ...
        coVerify(atLeast = 1) { localRepository.getAll().await() }
        coVerify(atLeast = 1) { jsonStorage.export(file, listOf(testTenista)).await() }
    }
    ...
    fun `export debe devolver error si no se puede exportar`() = runTest {
        ...
        } returns Err(TenistaError.StorageError("Error"))
