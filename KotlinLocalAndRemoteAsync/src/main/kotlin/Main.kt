...
class App : KoinComponent {
    fun run(): Unit = runBlocking {
        ...
        println("🔄 Refrescamos los tenistas automáticamente 🔄")
        tenistasService.enableAutoRefresh()

        delay(2000)

        // Obtenemos todos los tenistas
        var tenistas = tenistasService.getAll(false).await().mapBoth(
            success = {
        ...
        tenistasService.getById(1).await().mapBoth(
        ...
        tenistasService.save(newTenista).await().mapBoth(
            success = {
        ...
        // Obtenemos todos los tenistas
        tenistas = tenistasService.getAll(false).await().mapBoth(
            success = {
                println("Tenistas obtenidos: ${it.size}")
                println(it)
                it
        ...
        tenistasService.update(updatedTenista.id, updatedTenista).await().mapBoth(
            success = { println("Tenista actualizado: $it") },
            failure = { println(it.message) }
        )

        // Obtenemos todos los tenistas
        tenistasService.getAll(false).await().mapBoth(
            success = {
                println("Tenistas obtenidos: ${it.size}")
                println(it)
                it
        ...
        tenistasService.update(-1, updatedTenista).await().mapBoth(
            success = { println("Tenista actualizado: $it") },
            failure = { println(it.message) }
        )

        // Borramos un tenista
        tenistasService.delete(updatedTenista.id).await().mapBoth(
        ...
        tenistasService.delete(-1).await().mapBoth(
            success = { println("Tenista borrado") },
            failure = { println(it.message) }
        )
        ...
        tenistasService.import(csvImport).await().mapBoth(
        ...
        tenistasService.import(jsonImport).await().mapBoth(
        ...
        tenistasService.export(csvExport, fromRemote = true).await().mapBoth(
        ...
        tenistasService.export(jsonExport, fromRemote = true).await().mapBoth(
            success = { println("Tenistas exportados a json: $it") },
            failure = { println(it.message) }
        )

        // Consultamos todos los tenistas para hacerles las consultas
        tenistas = tenistasService.getAll(true).await().mapBoth(
            success = {
                println("Tenistas obtenidos: ${it.size}")
                println(it)
                it
