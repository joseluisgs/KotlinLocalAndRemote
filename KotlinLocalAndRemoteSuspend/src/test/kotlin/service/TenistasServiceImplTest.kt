...
class TenistasServiceImplTest {
    ...
    fun `getAll debe retornar lista de tenistas localmente`() = runTest {
        coEvery { localRepository.getAll() } returns Ok(listOf(testTenista))

        val result = service.getAll(false)
    ...
    fun `getAll lista de tenistas remota`() = runTest {
        coEvery { remoteRepository.getAll() } returns Ok(listOf(testTenista))
        coEvery { localRepository.saveAll(any()) } returns Ok(1)
        coEvery { localRepository.removeAll() } returns Ok(Unit)

        val result = service.getAll(true)
    ...
    fun `getById debe retornar tenista local si no esta en cache`() = runTest {
        ...
        coEvery { localRepository.getById(id) } returns Ok(testTenista)
    ...
    fun `getById debe retornar tenista remoto si no esta en cache ni local`() = runTest {
        val id = testTenista.id
        every { cache.get(id) } returns null
        coEvery { localRepository.getById(id) } returns Err(TenistaError.NotFound(id))
        coEvery { remoteRepository.getById(id) } returns Ok(testTenista)
        every { cache.put(id, testTenista) } returns Unit
        coEvery { localRepository.save(testTenista) } returns Ok(testTenista)
    ...
    fun `getById debe retornar error si no esta en cache ni local ni remoto`() = runTest {
        ...
        every { cache.get(id) } returns null
        coEvery { localRepository.getById(id) } returns Err(TenistaError.NotFound(id))
        coEvery { remoteRepository.getById(id) } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `save debe retornar tenista guardado`() = runTest {
        ...
        coEvery { localRepository.save(newTenista) } returns Ok(newTenista)
        coEvery { remoteRepository.save(newTenista) } returns Ok(newTenista)
        every { cache.put(newTenista.id, newTenista) } returns Unit
        coEvery { notificationsService.send(any()) } returns Unit

        val result = service.save(newTenista)
    ...
    fun `save debe retornar error si no se puede guardar remotamente`() = runTest {
        ...
        coEvery { remoteRepository.save(newTenista) } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `update debe retornar tenista actualizado`() = runTest {
        ...
        coEvery { localRepository.update(updatedTenista.id, updatedTenista) } returns Ok(updatedTenista)
        coEvery { remoteRepository.update(updatedTenista.id, updatedTenista) } returns Ok(updatedTenista)
        every { cache.put(updatedTenista.id, updatedTenista) } returns Unit
        coEvery { notificationsService.send(any()) } returns Unit

        val result = service.update(updatedTenista.id, updatedTenista)
    ...
    fun `update debe retornar error porque el tenista no existe remotamente`() = runTest {
        ...
        coEvery {
            localRepository.getById(updatedTenista.id)
        } returns Err(TenistaError.NotFound(updatedTenista.id))
        coEvery { remoteRepository.getById(updatedTenista.id) } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `delete debe retornar tenista borrado`() = runTest {
        ...
        // Suponemos que está en la cache
        every { cache.get(id) } returns testTenista
        coEvery { localRepository.delete(id) } returns Ok(id)
        coEvery { remoteRepository.delete(id) } returns Ok(id)
    ...
    fun `delete debe retornar error si no se puede borrar remotamente`() = runTest {
        ...
        every { cache.get(id) } returns testTenista
        coEvery { localRepository.delete(id) } returns Ok(id)
        coEvery { remoteRepository.delete(id) } returns Err(TenistaError.RemoteError("Error"))
    ...
    fun `delete debe retornar error si no se puede borrar localmente`() = runTest {
        ...
        every { cache.get(id) } returns testTenista
        coEvery { remoteRepository.delete(id) } returns Ok(id)
        coEvery { localRepository.delete(id) } returns Err(TenistaError.NotFound(id))
    ...
    fun `import debe devolver Ok para fichero CSV`() = runTest {
        ...
        coEvery { csvStorage.import(file) } returns Ok(listOf(testTenista))
        coEvery { localRepository.removeAll() } returns Ok(Unit)
        coEvery { remoteRepository.save(testTenista) } returns Ok(testTenista)
        coEvery { localRepository.save(testTenista) } returns Ok(testTenista)
    ...
    fun `import debe devolver Ok para fichero JSON`() = runTest {
        ...
        coEvery { jsonStorage.import(file) } returns Ok(listOf(testTenista))
        coEvery { localRepository.removeAll() } returns Ok(Unit)
        coEvery { remoteRepository.save(testTenista) } returns Ok(testTenista)
        coEvery { localRepository.save(testTenista) } returns Ok(testTenista)
    ...
    fun `import debe devolver error si no se puede importar`() = runTest {
        ...
        coEvery { csvStorage.import(file) } returns Err(TenistaError.StorageError("Error"))
    ...
    fun `export debe devolver Ok para fichero CSV de manera local`() = runTest {
        ...
        coEvery { localRepository.getAll() } returns Ok(listOf(testTenista))
        coEvery { csvStorage.export(file, listOf(testTenista)) } returns Ok(1)

        val result = service.export(file, false)
    ...
    fun `export debe devolver Ok para fichero CSV de manera remota`() = runTest {
        ...
        val result = service.export(file, true)
    ...
    fun `export debe devolver Ok para fichero Json de manera local`() = runTest {
        ...
        coEvery { localRepository.getAll() } returns Ok(listOf(testTenista))
        coEvery { jsonStorage.export(file, listOf(testTenista)) } returns Ok(1)
