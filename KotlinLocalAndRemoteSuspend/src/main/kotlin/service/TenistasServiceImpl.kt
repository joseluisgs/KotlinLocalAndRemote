...
class TenistasServiceImpl(
    ...
    override suspend fun loadData() {
        ...
        localRepository.removeAll()
            .andThen { remoteRepository.getAll() } // Obtenemos los datos remotos
            .andThen { localRepository.saveAll(it) }.also {
    ...
    override suspend fun getAll(fromRemote: Boolean): Result<List<Tenista>, TenistaError> =
        withContext(Dispatchers.IO) {
            logger.debug { "Obteniendo todos los tenistas" }
            // Devolvemos los datos locales
            if (!fromRemote) {
                localRepository.getAll().mapBoth(
                    success = { Ok(it) },
                    failure = { Err(it) }
                )
            } else {
                localRepository.removeAll() // Borramos los datos locales
                    .andThen { remoteRepository.getAll() } // Obtenemos los datos remotos
                    .andThen { localRepository.saveAll(it) }
                    .andThen { localRepository.getAll() }.mapBoth(
                        success = { Ok(it) },
                        failure = { Err(it) }
    ...
    override suspend fun getById(id: Long): Result<Tenista, TenistaError> = withContext(Dispatchers.IO) {
        ...
        cache.get(id)?.let {
            ...
            localRepository.getById(id).mapBoth(
                success = {
                    cache.put(it.id, it)
                    Ok(it)
                },
                failure = {
                    ...
                    remoteRepository.getById(id).andThen { localRepository.save(it) }.mapBoth(
                        success = {
                            cache.put(it.id, it)
                            Ok(it)
                        },
    ...
    override suspend fun save(tenista: Tenista): Result<Tenista, TenistaError> = withContext(Dispatchers.IO) {

        logger.debug { "Guardando tenista: $tenista" }
        // validamos
        tenista.validate()
            .andThen { remoteRepository.save(tenista) }
            .andThen { localRepository.save(it) }.mapBoth(
                success = {
    ...
    override suspend fun update(id: Long, tenista: Tenista): Result<Tenista, TenistaError> =
        withContext(Dispatchers.IO) {

            logger.debug { "Actualizando tenista por id: $id" }
            // validamos
            tenista.validate()
                .andThen { getById(id) } // No es porque va a fallar en remoto si no existe, es un ejemplo para concatenar cosas
                .andThen { remoteRepository.update(id, tenista) }
                .andThen { localRepository.update(id, tenista) }
                .mapBoth(
                    success = {
    ...
    override suspend fun delete(id: Long): Result<Long, TenistaError> = withContext(Dispatchers.IO) {
        ...
        getById(id)
            ...
            .andThen { localRepository.delete(id) }.mapBoth(
                success = {
    ...
    override suspend fun import(file: File): Result<Int, TenistaError> = withContext(Dispatchers.IO) {
    ...
    private suspend fun importCsv(file: File): Result<Int, TenistaError> {
        ...
        return csvStorage.import(file).mapBoth(
            success = { saveAll(it) },
            failure = { Err(it) }
        )
    }
    ...
    private suspend fun importJson(file: File): Result<Int, TenistaError> {
        ...
        return jsonStorage.import(file).mapBoth(
            success = { saveAll(it) },
            failure = { Err(it) }
        )
    }
    ...
    private suspend fun saveAll(tenistas: List<Tenista>): Result<Int, TenistaError> {
        ...
        var contador = 0
        localRepository.removeAll().mapBoth(
            success = {
                tenistas.forEach { tenista ->
                    remoteRepository.save(tenista)
                        .andThen { localRepository.save(it) }
                        .andThen { Ok(contador++) }
                }
            },
    ...
    override suspend fun export(file: File, fromRemote: Boolean): Result<Int, TenistaError> =
        withContext(Dispatchers.IO) {
    ...
    private suspend fun exportCsv(file: File, fromRemote: Boolean = false): Result<Int, TenistaError> {
        ...
        return if (fromRemote) {
            remoteRepository.getAll().andThen { tenistas ->
                csvStorage.export(file, tenistas)
            }
        } else {
            localRepository.getAll().andThen { tenistas ->
                csvStorage.export(file, tenistas)
            }
        }
    }
    ...
    private suspend fun exportJson(file: File, remote: Boolean = false): Result<Int, TenistaError> {
        ...
        return if (remote) {
            remoteRepository.getAll().andThen { tenistas ->
                jsonStorage.export(file, tenistas)
            }
        } else {
            localRepository.getAll().andThen { tenistas ->
                jsonStorage.export(file, tenistas)
