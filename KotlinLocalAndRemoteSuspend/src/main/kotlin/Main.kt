...
class App : KoinComponent {
    fun run(): Unit = runBlocking {
        ...
        var tenistas = tenistasService.getAll(false).mapBoth(
            success = {
        ...
        tenistasService.getById(1).mapBoth(
        ...
        tenistasService.save(newTenista).mapBoth(
            success = {
        ...
        // Obtenemos todos los tenistas
        tenistas = tenistasService.getAll(false).mapBoth(
            success = {
                println("Tenistas obtenidos: ${it.size}")
                println(it)
                it
        ...
        tenistasService.update(updatedTenista.id, updatedTenista).mapBoth(
            success = { println("Tenista actualizado: $it") },
            failure = { println(it.message) }
        )

        // Obtenemos todos los tenistas
        tenistasService.getAll(false).mapBoth(
            success = {
                println("Tenistas obtenidos: ${it.size}")
                println(it)
                it
        ...
        tenistasService.update(-1, updatedTenista).mapBoth(
            success = { println("Tenista actualizado: $it") },
            failure = { println(it.message) }
        )

        // Borramos un tenista
        tenistasService.delete(updatedTenista.id).mapBoth(
        ...
        tenistasService.delete(-1).mapBoth(
            success = { println("Tenista borrado") },
            failure = { println(it.message) }
        )
        ...
        tenistasService.import(csvImport).mapBoth(
        ...
        tenistasService.import(jsonImport).mapBoth(
        ...
        tenistasService.export(csvExport, fromRemote = true).mapBoth(
