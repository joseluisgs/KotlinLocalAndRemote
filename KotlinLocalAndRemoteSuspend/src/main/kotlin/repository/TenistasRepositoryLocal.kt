...
class TenistasRepositoryLocal(
    ...
    override suspend fun getAll(): Result<List<Tenista>, TenistaError> = withContext(Dispatchers.IO) {
        logger.debug { "Obteniendo todos los tenistas de la bd" }
        Ok(sqlClient.queries.selectAll().executeAsList().map { it.toTenista() })
    }
    ...
    override suspend fun save(t: Tenista): Result<Tenista, TenistaError> = withContext(Dispatchers.IO) {
    ...
    override suspend fun update(id: Long, t: Tenista): Result<Tenista, TenistaError> = withContext(Dispatchers.IO) {
    ...
    suspend fun removeAll(): Result<Unit, TenistaError> = withContext(Dispatchers.IO) {
    ...
    suspend fun saveAll(tenistas: List<Tenista>): Result<Int, TenistaError> = withContext(Dispatchers.IO) {
